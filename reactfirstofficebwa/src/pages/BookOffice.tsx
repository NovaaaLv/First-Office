...
export default function BookOffice() {
	...
	useEffect(() => {
		...
		axios
			.get(`http://127.0.0.1:8000/api/office/${slug}`, {
				headers: {
					"X-API-KEY": "abcdefghijklmnopqrstu",
				},
			})
			.then((response) => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!validation.success) {
			console.log("Validating errors: ", validation.error.issues);
		...
		try {
			...
			if (axios.isAxiosError(error)) {
				console.log("Error submiting form: ", error.message);
				setError(error.message);
			} else {
				console.error("Unexpected error : ", error);
					...
					<div className="flex flex-col gap-4">
						...
						<div className="flex flex-col gap-2">
							...
							<div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#0D903A] overflow-hidden">
								...
								) && <p className="text-red-500">Started date is required</p>}
							</div>
						</div>
					</div>
					<hr className="border-[#F6F5FD]" />
